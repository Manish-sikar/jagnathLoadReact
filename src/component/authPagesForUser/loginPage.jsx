...
import { UserLoginApi, ForgotPasswordApi, VerifyOtpApi, ResetPasswordApi } from "../../services/authServices";
        ...
        <MDBCol col="4" md="6">
          ...
          <MDBModal open={modalOpen} tabIndex="-1" setOpen={setModalOpen}>
            ...
            <MDBModalBody>
              {step === 1 && (
                <>
                  <MDBInput label="Enter your email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="mb-3" />
                  <MDBBtn onClick={handleForgotPassword}>Send OTP</MDBBtn>
                </>
              )}
              {step === 2 && (
                <>
                  <MDBInput label="Enter OTP" type="text" value={otp} onChange={(e) => setOtp(e.target.value)} className="mb-3" />
                  <MDBBtn onClick={handleVerifyOtp}>Verify OTP</MDBBtn>
                </>
              )}
              {step === 3 && (
                <>
                  <MDBInput label="New Password" type="password" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} className="mb-3" />
                  <MDBInput label="Confirm Password" type="password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} className="mb-3" />
